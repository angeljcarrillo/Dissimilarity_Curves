function alfaF=alfaSimFC(A,B,epsilon,segByLCM,D) %% A(X,Y)=B, EPSILON(0-1). #SEGMENTO, DECIMALES
%% COLORES Y SIMBOLOS
...
p22=B;
%% SCC
vA=anguloContingencia2(p11(:,2),p11(:,1),0);
vB=anguloContingencia2(p22(:,2),p22(:,1),0);
%% AJUSTE PARA CURVAS CERRADAS EN SCC Y EN PUNTOS
vA(:,end)=[];
vB(:,end)=[];
...
vA=outN{:,1};
%% PRECIION EN EL CODIGO
vA = round(vA,D);
vB = round(vB,D);

%% 
subCA=subChain5(vA,epsilon,p11);
esccASC=subCA{:,1};
vASC=subCA{:,2};
%% VISTA
cont1C=1;
...
%%
subCB=subChain5(vB,epsilon,p22);
esccBSC=subCB{:,1};
vBSC=subCB{:,2};
%% VISTA
cont1C=1;
...
for u=1:1:length(subAN2SCC)
    ...
    dif2=sum(abs(AChain2-BChain2)/2);
    muBA=[muBA(:,:) dif];
    muBA2=[muBA2(:,:) dif2];
end
...
dis2=abs(length(muAB2)-length(muBA2));
maxSUB=segByLCM;
difSUB=dis2/maxSUB;
